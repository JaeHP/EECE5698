...
## Class initialization    
    def __init__(self,port_name):
        ...
        self.lc = lcm.LCM()
        self.data = bot_messages.gpsdata()
        print 'Init  complete'
        ...
        while True:
            ...
            if line.startswith('$GPGGA'):
                try:
                    ...
                    self.data.latitude = (int(vals[2][:2]) + float(vals[2][2:])/60) * lat_sign  #convert degree minutes format to decimal degrees
                    self.data.longitude = (int(vals[4][:3]) + float(vals[4][3:])/60) * lon_sign
                    self.data.altitude = float(vals[9])
                    utm_data = utm.from_latlon(self.data.latitude,self.data.longitude)
